...
class FeatureSelector:
    ...
    The recommended path is: Normalization --> Feature selection
    """
    def __init__(self, df: pd.DataFrame):
        ...
        ----------
        df: pd.DataFrame
            A dataframe with only continous data describing the observations and features
        """
        self.df = df

        
    def scale_data(self: FeatureSelector, verbose: bool=False) -> pd.DataFrame:
        ...
        df_normalized = pd.DataFrame(df_normalized, columns=self.df.columns)
        if verbose:
            print("===== DESCRIPTION =====")
