...
def test_scale_data(sample_data):
    ...
    assert 'Log_MP_RATIO' in normalized_df.columns, "Dependent variable should not be removed"
    assert normalized_df['feature1'].min() >= 0 and normalized_df['feature1'].max() <= 1, "Data should be normalized"


def test_remove_low_variance(sample_data):
    fs = FeatureSelector(sample_data)
    cleaned_df, deleted_features = fs.remove_low_variance(variance_threshold=0.01, verbose=False, inplace=False)
    assert isinstance(cleaned_df, pd.DataFrame), "Should return a DataFrame"
    assert isinstance(deleted_features, list), "Should return a list of deleted features"
...
def test_get_correlation(sample_data):
    ...
    assert correlation_matrix.shape == (3, 3), "Correlation matrix should not include the dependent variable"


def test_remove_highly_correlated(sample_data):
    fs = FeatureSelector(sample_data)
    cleaned_df = fs.remove_highly_correlated(threshold=0.9, verbose=False, inplace=False)
    assert isinstance(cleaned_df, pd.DataFrame), "Should return a DataFrame"
