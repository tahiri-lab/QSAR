...
from qsar.preprocessing.custom_preprocessing import LowVarianceRemover, HighCorrelationRemover, PreprocessingPipeline
...
def test_low_variance_remover(sample_data):
    transformer = LowVarianceRemover(y='Log_MP_RATIO', variance_threshold=0.01, cols_to_ignore=[], verbose=False)
...
def test_high_correlation_remover(sample_data):
    transformer = HighCorrelationRemover(df_correlation=None, df_corr_y=None, threshold=0.9, verbose=False)
    transformed_data = transformer.transform([sample_data])
    assert isinstance(transformed_data, pd.DataFrame), "Should return a DataFrame"


def test_preprocessing_pipeline(sample_data):
    pipeline = PreprocessingPipeline(target='Log_MP_RATIO', variance_threshold=0.01, cols_to_ignore=[], verbose=False,
                                     threshold=0.9)
    ...
    assert isinstance(transformed_data, pd.DataFrame), "Pipeline should output a DataFrame"
