...
def test_extract_descriptors(sample_molecules):
    ...
    assert df.shape[0] == len(sample_molecules), "DataFrame should have one row per molecule"
    assert df.shape[1] == len(Descriptors.descList), "DataFrame should have one column per descriptor"
